...
async function list(req, res) {
  var productos = (query) => {
    return new Promise((resolve, reject) => {
      mysqlConnection.getConexion("producto", null)
...
async function save(req, res) {
  ...
  mysqlConnection.getConexion("producto", parseInt(data.id_sucursal))
    .then(sqlconnection => {
        ...
        .then(async sqlconnection => {
          ...
          if (Array.isArray(sqlconnection)) {
            for (let conection of sqlconnection) {
              ...
              var resultProductosSitio = await productosSitio();
              if (resultProductosSitio.length > 0)
                return resolve(resultProductosSitio[0]);
            }
          }
          else {
            const tabla = sqlconnection.tabla;
